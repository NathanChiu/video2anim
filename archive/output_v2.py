...
def content_loss(p,x):
    ...
    loss = (1. / (4 * N * M)) * tf.reduce_sum(input_tensor=tf.pow(x - p, 2))
    return loss
...
def style_loss_func(sess, model):
    ...
    def _gram_matrix(F, N, M):
        Ft = tf.reshape(F, (M, N))
        return tf.matmul(tf.transpose(a=Ft), Ft)
    ...
    def _style_loss(a, x):
        ...
        E_l = (1 / (4 * N**2 * M**2)) * tf.reduce_sum(input_tensor=tf.pow(G - A, 2))
        return E_l
...
for it in range(iterations+1):
    ...
    if it%200 == 0:
        ...
        print('Iteration %d' % (it))
        print('sum : ', sess.run(tf.reduce_sum(input_tensor=img_mixed)))
        print('cost: ', sess.run(total_loss))

        filename = 'output_v2/%d.png' % (it)
