...
class StyleTransfer():
    def __init__(self, content_path, style_path):
        ...
        self.output_file_dir = os.path.join("output_images", timestr)
    ...
    def run(self, num_iterations):
        ...
        content = self.load_img(self.content_path).astype('uint8')
        style = self.load_img(self.style_path).astype('uint8')


        content_save = np.squeeze(content, axis=0)
        # Normalize for display
        content_save = content_save.astype('uint8')
        # cv2.imwrite(os.path.join(self.output_file_dir, "content.png"), content_save)
        cv2.imwrite(os.path.join(self.output_file_dir, "content.png"), cv2.cvtColor(content_save, cv2.COLOR_RGB2BGR))

        style_save = np.squeeze(style, axis=0)
        # Normalize for display
        style_save = style_save.astype('uint8')
        # cv2.imwrite(os.path.join(self.output_file_dir, "style.png"), style_save)
        cv2.imwrite(os.path.join(self.output_file_dir, "style.png"), cv2.cvtColor(style_save, cv2.COLOR_RGB2BGR))
        ...
        best, best_loss = self.run_style_transfer(num_iterations=num_iterations)
        # cv2.imwrite(os.path.join(self.output_file_dir, "best.png"), best)
        cv2.imwrite(os.path.join(self.output_file_dir, "best.png"), cv2.cvtColor(best, cv2.COLOR_RGB2BGR))
    ...
    def get_feature_representations(self, model):
        ...
        style_features = [style_layer[0] for style_layer in style_outputs[:len(self.style_layers)]]
        content_features = [content_layer[0] for content_layer in content_outputs[len(self.style_layers):]]
    ...
    def compute_loss(self, model, loss_weights, init_image, gram_style_features, content_features):
        ...
        style_output_features = model_outputs[:len(self.style_layers)]
        content_output_features = model_outputs[len(self.style_layers):]
    ...
    def run_style_transfer(self,
      ...
      # trainable to false.
        ...
        prog_bar = tf.keras.utils.Progbar(num_iterations)
        for i in range(num_iterations):
            ...
            if i % display_interval == 0:
                ...
                # cv2.imwrite(os.path.join(self.output_file_dir, f"{i}.png"), plot_img)
                cv2.imwrite(os.path.join(self.output_file_dir, f"{i}.png"), cv2.cvtColor(plot_img, cv2.COLOR_RGB2BGR))
    ...
    def show_results(self, best_img, show_large_final=True):
        plt.figure(figsize=(10, 5))
        content = self.load_img(self.content_path)
        style = self.load_img(self.style_path)
